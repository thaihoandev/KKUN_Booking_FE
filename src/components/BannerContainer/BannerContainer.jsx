...
function BannerContainer() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!selectedDestination) {
            showToast("error", TOAST_MESSAGES.REQUIRED_LOCATION);
            return;
        }

        if (!dateRange) {
            showToast("error", TOAST_MESSAGES.REQUIRED_DATE);
            return;
        }
        ...
        if (adultQty < 1) {
            showToast("error", TOAST_MESSAGES.REQUIRED_ADULTS);
            return;
        }
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                showToast("error", TOAST_MESSAGES.SEARCH_ERROR);
            }
        } catch (error) {
            dismissToast(loadingToastId);
            showToast("error", `${TOAST_MESSAGES.API_ERROR}: ${error.message}`);
        }
    };
