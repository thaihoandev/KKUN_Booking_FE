...
function BannerContainer() {
    ...
    const [errors, setErrors] = useState({ location: "", daterange: "" });
    ...
    const handleClickOutside = (e) => {
        if (dropdownRef.current && !dropdownRef.current.contains(e.target)) {
            setDropdownOpenGuest(false);
        }
        if (
            dropdownSearchRef.current &&
            !dropdownSearchRef.current.contains(e.target)
        ) {
            setDropdownOpen(false);
        }
    };
    ...
    // Guest quantity state and functions
    const [adultQty, setAdultQty] = useState(1);
    const [childQty, setChildQty] = useState(0);
    const handleIncreaseGuest = (type) => {
        type === "adult"
            ? setAdultQty((prevQty) => prevQty + 1)
            : setChildQty((prevQty) => prevQty + 1);
    };
    const handleDecreaseGuest = (type) => {
        if (type === "adult" && adultQty > 1)
            setAdultQty((prevQty) => prevQty - 1);
        if (type === "child" && childQty > 0)
            setChildQty((prevQty) => prevQty - 1);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        setErrors({ location: "", daterange: "" });

        // Validate location và date range
        let hasErrors = false;
        let newErrors = { location: "", daterange: "" };
        ...
        if (!dateRange || !dateRange.includes(" - ")) {
            newErrors.daterange = "Vui lòng chọn khoảng thời gian hợp lệ";
            hasErrors = true;
        }

        // Nếu có lỗi, đặt trạng thái lỗi và dừng việc submit form
        if (hasErrors) {
            setErrors(newErrors);
            return;
        }

        // Tách ngày checkIn và checkOut từ dateRange
        const [checkInDate, checkOutDate] = dateRange.split(" - ");

        // Định dạng ngày với giờ check-in là 14:00 và giờ check-out là 12:00
        ...
        try {
            ...
            if (response.status === 200) {
                // Sau khi API thành công, chuyển hướng tới trang danh sách khách sạn
            ...
            console.log("Lỗi kết nối:", error.message); // Log lỗi chi tiết hơn
        }
    };
                                        ...
                                        <div className="filter-area">
                                            <div className="row g-xl-4 gy-4">
                                                ...
                                                <input
                                                    type="hidden"
                                                    name="location"
                                                    value={selectedDestination} // Set hidden input value to selected destination
                                                />
                                                <div className="col-xl-3 col-md-6 d-flex justify-content-center divider">
                                                    <div className="single-search-box">
                                                        <div className="icon">
                                                        ...
                                                        <div className="searchbox-input">
                                                            ...
                                                            <div className="quantity-counter">
                                                                ...
                                                                <a
                                                                    ...
                                                                    onClick={
                                                                        handleIncrease
                                                                    } // Gọi hàm tăng số lượng khi nhấn nút
                                                                >
